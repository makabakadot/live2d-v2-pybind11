...
void live2d::Render::setMatrix(const float m[16])
{
	std::memcpy(mMatrix4x4.data(), m, 16 * sizeof(float));
}

void live2d::Render::setClipMatrix(const float m[16])
{
	std::memcpy(mClipMatrix.data(), m, 16 * sizeof(float));
}
void live2d::Render::setCulling(bool b)
{
    mCulling = b;
}
live2d::ClipContext* live2d::Render::findSameClip(const std::vector<ClipContext*>& list, const std::vector<std::string>& ids)
{
...
void live2d::Render::init(ModelContext* mc, const std::vector<IDrawData*>& drawDataList, const std::vector<MeshContext*>& drawContextList)
{
    int clipCount = 0;
...
void live2d::Render::calcClippedDrawTotalBounds(ModelContext* mc, ClipContext* clip)
{
...
void live2d::Render::setupLayoutBounds(int count)
{
...
void live2d::Render::buildClipMatrix(std::array<float, 16>& out, ClipContext* clip, bool forMask)
{
    out.fill(0);
...
void live2d::Render::setupClip(ModelContext* mc)
{
    int activeCount = 0;
    ...
    for (auto* clip : mClipContextList)
    {
        ...
        for (int idx : clip->mClippingMaskDrawIndexList)
        {
            ...
            auto* ctx = mc->getDrawContext(idx);
            this->setClipMatrix(clip->mMatrixForMask.data());
            this->mClipChannel = clip->mLayoutChannelNo;
            this->setClipBufPre_clipContextForMask((void*)1);
            dd->draw(this, mc, ctx);
            this->setClipBufPre_clipContextForMask(nullptr);
        }
    }
    this->exitMaskRendering();
}
