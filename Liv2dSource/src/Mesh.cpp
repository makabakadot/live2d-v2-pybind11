...
void Mesh::draw(Render*  render, ModelContext * mctx, MeshContext * dctx)//之中不增加引用与释放
{
    ...
    if (dctx->mClipBufPre_clipContext) {
        ...
        render->mClipChannel = cc->mLayoutChannelNo;
        render->setClipMatrix(cc->mMatrixForDraw.data());
    }
    render->setClipBufPre_clipContextForDraw(dctx->mClipBufPre_clipContext);
    render->setCulling(mCulling);

    auto* pctx2 = mctx->getPartsContext(dctx->mPartsIndex);
    float opacity = getOpacity(dctx) * dctx->mPartsOpacity * dctx->mBaseOpacity;
    render->drawTexture(texNr, pctx2->mScreenColor, mIndexArray, vertices, mUvs,
