...
void* BinaryReader::readObjectUntyped(int type) {
    ...
    //mObjects->push_back(obj); void* obj = readKnownTypeObject(type);中返回
    return obj;
}
...
void* BinaryReader::readKnownTypeObject(int type) {
    ...
    if (type == 16 || type == 25) {
        ...
        mObjects->push_back(arr);
        mDeleters->push_back([](void* ptr) { delete static_cast<std::vector<int32_t>*>(ptr); });
        return arr;
    }
    if (type == 26) {
        auto* arr = new std::vector<double>(readFloat64Array());
        mObjects->push_back(arr);
        mDeleters->push_back([](void* ptr) { delete static_cast<std::vector<double>*>(ptr); });
        return arr;
    }
    if (type == 27) {
        auto* arr = new std::vector<float>(readFloat32Array());
        mObjects->push_back(arr);
        mDeleters->push_back([](void* ptr) { delete static_cast<std::vector<float>*>(ptr); });
        return arr;
    }
    if (type == 23) {
