...
LAppModel::LAppModel(std::shared_ptr<Render> render):L2DBaseModel(render)
{
...
bool LAppModel::loadModelJson(const std::string& path) {
...
#ifdef LOGGING
    std::cout << "Load model : "<< mocPath << std::endl;
#endif // LOGGING
    ...
    for (size_t i = 0; i < texPaths.size(); i++) {
        ...
        std::shared_ptr<Render> dp = mLive2DModel->getDrawParam();
        if (dp)
            dp->setTexture(i, texData);
    }
    ...
    if (poseKeyPos != std::string::npos) {
        ...
        if (q2 != std::string::npos) {
            ...
            if (!poseData.empty()) {
#ifdef LOGGING
                std::cout << "Load pose : " << poseFile << std::endl;
#endif // LOGGING
                delete mPose;
...
#ifdef LOGGING
                    std::cout << "Load motion : " << motFile << std::endl;
#endif // LOGGING
...
#ifdef LOGGING
                    std::cout << "Load expression : " << expName << std::endl;
#endif // LOGGING
...
void LAppModel::draw() {
    ...
    std::shared_ptr<Render> render = mLive2DModel->getDrawParam();
    auto mvp = mMatrixManager.getMvp(&mModelMatrix);
    render->setMatrix(mvp.data());
    mLive2DModel->getModelContext()->preDraw(render);
    mLive2DModel->draw();
}
...
#ifdef LOGGING
        std::cout << "Load expression : " << "Start expression:"<< name << std::endl;
#endif // LOGGING
...
#ifdef LOGGING
        std::cout << "Load expression : " << "Start random expression: " << it->first.c_str() << std::endl;
#endif // LOGGING
        mExpressionMgr->startMotion(it->second, false);
    }
}
