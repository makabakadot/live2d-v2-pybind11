...
void ModelContext::release() {
    ...
    //delete mDpGL; Live2DModelOpenGL中删除
    this->render = nullptr;
...
void ModelContext::init() {
    ...
    this->render->init(this, mDrawDataList, mDrawContextList);
...
#endif // V2CPP_XFORM_DUMP
    for (int i = static_cast<int>(mUpdatedParamFlags.size()) - 1; i >= 0; i--)
        mUpdatedParamFlags[i] = false;
    mNeedSetup = false;
}

void ModelContext::preDraw(std::shared_ptr<Render> render)
{
    if (render) { render->setupDraw();  render->setupClip(this); }
}

void ModelContext::draw(std::shared_ptr<Render>  render)
{
