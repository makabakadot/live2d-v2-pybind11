...
inline T BinaryReader::readObjectVector(int type) {
    ...
    if constexpr (std::is_pointer_v<ElemType>) {
        ...
        if (actualType == OBJECT_REF) {
            ...
            auto* storedVec = static_cast<T*>(mObjects->at(index));
            return *storedVec;
        }
        ...
        mDeleters->push_back([](void* ptr) { delete static_cast<T*>(ptr); });
        return *resultPtr;
        ...
        if (actualType == OBJECT_REF) {
            ...
            return *static_cast<T*>(mObjects->at(index));
        }
        if (actualType != 15) {
        ...
        //mObjects.push_back(obj);//readKnownTypeObject中加入mObjects
